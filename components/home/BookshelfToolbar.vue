  ...
  <div class="w-full h-9 bg-bg relative z-20">
    <div id="bookshelf-toolbar" class="absolute top-0 left-0 w-full h-full z-20 flex items-center px-2">
      <div class="flex items-center w-full text-sm">
        ...
        <div class="flex-grow" />
        <span v-if="page == 'library' || seriesBookPage" tabindex="0" class="material-symbols text-2xl px-2" @click="changeView" @keydown.enter="changeView">{{ !bookshelfListView ? 'view_list' : 'grid_view' }}</span>
        <template v-if="page === 'library'">
          <div class="relative flex items-center px-2" tabindex="0" @keydown.enter="showFilterModal = true" @click="showFilterModal = true">
          ...
          <span tabindex="0" class="material-symbols text-2xl px-2" @click="showSortModal = true" @keydown.enter="showSortModal = true">sort</span>
        </template>
        <span v-if="seriesBookPage" tabindex="0" class="material-symbols text-2xl px-2" @click="downloadSeries" @keydown.enter="downloadSeries">download</span>
        <span v-if="(page == 'library' && isBookLibrary) || seriesBookPage" tabindex="0" class="material-symbols text-2xl px-2" @click="showMoreMenuDialog = true" @keydown.enter="showMoreMenuDialog = true">more_vert</span>
      </div>
    </div>
